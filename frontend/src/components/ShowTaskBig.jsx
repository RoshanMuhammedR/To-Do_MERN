...
export const ShowTaskBig = ({ choosenTask }) => {
  ...
  return (
    <div className="w-full flex flex-col justify-between h-full py-6 text-black">
      <form onSubmit={(e) => e.preventDefault()} className="flex flex-col gap-y-8">
        ...
        <div className="flex flex-wrap gap-8">
          ...
          <div>
            <label className="text-lg font-medium block mb-1">Section</label>
            <select
              className="border border-gray-400 p-2 rounded text-base focus:outline-none"
              value={task.section}
              onChange={(e) => setTask({ ...task, section: e.target.value })}
            >
              <option value="task">Task</option>
              <option value="calendar">Calendar</option>
              <option value="sticky-wall">Sticky Wall</option>
            </select>
          </div>
        </div>
